...
//Objective function
minimize r + (1 / C_intra) * (sum (i in N) xi_intra[i]) +  (1 / C_inter) * (sum (i in N) xi_inter[i]);
...
subject to {
	forall(i in N, j in N){
		if (y[i] == y[j]) {
			sum (k in M) a[i][j][k] * w[k] <= r + xi_intra[i] + xi_intra[j];
		} else {
			sum (k in M) a[i][j][k] * w[k] >= 1 - xi_inter[i] - xi_inter[j];
		}
	}
}
