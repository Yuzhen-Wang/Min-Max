...
//Objective function
minimize r + (1 / C_intra) * (sum (i in N, j in N) xi_intra[i][j]) +  (1 / C_inter) * (sum (i in N, j in N) xi_inter[i][j]);
...
subject to {
	forall(i in N, j in N){
		if (y[i] == y[j]) {
			sum (k in M) a[i][j][k] * w[k] <= r + xi_intra[i][j];
		} else {
			sum (k in M) a[i][j][k] * w[k] >= 1 - xi_inter[i][j];
		}
	}
}
